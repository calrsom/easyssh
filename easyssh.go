...
func (ssh_conf *MakeConfig) Upload(sourceFile string, destinationDir string) error {
	_, session, err := ssh_conf.connect()
	...
	if err := session.Run(fmt.Sprintf("cd %s && scp -t %s", destinationDir, targetFile)); err != nil {
		return err
	}
